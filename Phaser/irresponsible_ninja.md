...
- [Phaser 小游戏 —— 不靠谱的忍者](#phaser-小游戏--不靠谱的忍者)
  - [单独一个 `Scene` 作为背景，防止显示 `Scene` 重叠时有遮挡](#单独一个-scene-作为背景防止显示-scene-重叠时有遮挡)
  - [底部组件](#底部组件)
    - [底部的云层效果作为公共组件使用](#底部的云层效果作为公共组件使用)
    - [添加动画](#添加动画)
    - [适配，始终显示在窗口底部，css 的 fixed 效果](#适配始终显示在窗口底部css-的-fixed-效果)
  - [开始页面](#开始页面)
  - [主场景](#主场景)
    - [显示静态信息](#显示静态信息)
    - [生成站台](#生成站台)
